...
mass_helium =    4.00            # mass in u = 1e-3 kg/mol
sigma_helium =   0.25            # sigma in nm     Helium: 0.25
epsilon_helium = 10.2*R*1e-3     # epsilon in kJ/mol Helium: 10.2
...
# output
file_name_base = "my_simulation_Amelie_Test"  # file name for all output files
...
ps2 = ParticleSystem2(n_particles)

# fill in the parameters for argon & helium


for i in range(n_particles):
    ps.set_parameters( i, mass=mass_argon, sigma=sigma_argon, epsilon=epsilon_argon)   
    ps2.set_parameters(i, mass=mass_helium, sigma=sigma_helium, epsilon=epsilon_helium)



# set initial positions     
initialize_positions(ps, sim.box_length)
initialize_positions(ps2, sim.box_length)

# set initial velocities     
initialize_velocities(ps, sim.temperature)
initialize_velocities(ps2, sim.temperature)
...
E_pot_init2 = potential_energy(ps2, sim)
E_kin_init2 = kinetic_energy(ps2)
T_init2 = instantaneous_temperature(ps2)
P_init2 = ideal_gas_pressure(ps2, sim)
...
position_trajectory2 = np.zeros((sim.n_steps+1, n_particles, 3))
position_trajectory2[0,:,:] = ps2.position # initial position
...
# initialize energy trajectory
energy_trajectory2 = np.zeros((sim.n_steps+1, 4))
energy_trajectory2[0,0] = potential_energy(ps2, sim)       # potential energy
energy_trajectory2[0,1] = kinetic_energy(ps2)               # kinetic energy
energy_trajectory2[0,2] = instantaneous_temperature(ps2)    # instantaneous pressure
energy_trajectory2[0,3] = ideal_gas_pressure(ps2, sim)      # ideal gas pressure
...
for i in range(sim.n_steps):
    if NVT==True:
        simulate_NVT_step(ps2, sim)
    else: 
        simulate_NVE_step(ps2, sim)
        
    # store updated positions
    position_trajectory2[i+1,:,:] = ps2.position # store updated positions

    # store updated energies, temperature and pressure
    energy_trajectory2[i+1,0] = potential_energy( ps2, sim)     # potential energy
    energy_trajectory2[i+1,1] = kinetic_energy(ps2)             # kinetic energy
    energy_trajectory2[i+1,2] = instantaneous_temperature(ps2)  # instantaneous pressure
    energy_trajectory2[i+1,3] = ideal_gas_pressure(ps2, sim)    # ideal gas pressure
...
write_xyz_trajectory(file_name_base + "_pos2.xyz", position_trajectory2, atom_symbol="He")
