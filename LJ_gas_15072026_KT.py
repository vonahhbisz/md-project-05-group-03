    ...
    def set_parameters(self, i, mass, sigma, epsilon, atom_type=None):
        """Set the paramters of the i-th particle
            ...
            atom_type: optional element/species label (e.g. "Ar", "Kr"),
                       if None, the label is left unchanged (default "X").
        """
        self.mass[i] = mass
        self.sigma[i] = sigma
...
# this is only really useful when we assume the same properties for both atom types but want different labels
...
def combine_lj_parameters(ps: ParticleSystem):
    """
    computes sigma_ij, epsilon_ij using the Lorentz-Berthelot combining rules:

        sigma_ij   = (sigma_i + sigma_j) / 2          (arithmetic mean)
        epsilon_ij = sqrt(epsilon_i * epsilon_j)      (geometric mean)

    single-species system: reduces to pure sigma and epsilon of that species 
    (sigma_ii = sigma_i, epsilon_ii = epsilon_i)
    """
...
def potential_energy(ps: ParticleSystem, sim: SimulationParameters) -> float:
    """
    computes total Lennard-Jones potential energy of the system using Lorentz-Berthelot combining rules
